...
Currently, Cwerg only works on recent inux systems.
It has been primarily tested on Ubuntu 24.04.

Be prepared for a bumpy ride - Cwerg is very much work-in-progress.
...
* The compiler driver will then be available at `Cwerg/cwereg.py`
  Run: `Cwerg/cwerg.py -be py -fe py Cwerg/FE/TestData/hello_world_test.cw hello.exe`
  Followed by `./hello.exe`
...
* The compilation in the previous step was kind of slow because it uses
  the Python version of the front- and backend. To use the faster C++ versions
  we must build them first.
  Run: `cd Cwerg/ ; make build_compiler`
  Note: you may need to install the `make cmake g++ libunwind-dev` packages first

* Now you can run the much fast compiler like so:
