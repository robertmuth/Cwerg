...
// ImmutablePool is used to store immutable byte sequences. If the byte sequence
// has been stored before it will not be stored again. This makes it suitable
// got interning strings. The Byte sequence are identified by a uint32_t offset
// which will never change. The offset is guaranteed to be aligned by
// `alignment`. Once stored a byte sequences cannot be deleted from the pool, so
// the pool keeps growing. The accumulated size of unique byte sequences stored
// must not overflow 32 bits (about 4GB).
class ImmutablePool {
 public:
...
// As above but also records the size
class ImmutablePoolWithSizeInfo {
 public:
  ImmutablePoolWithSizeInfo(size_t alignment)
      : data_(new char[16 * 1024 * 1024]),
        reserved_(16 * 1024 * 1024),
        alignment_(alignment) {
    // Fill first slot so that 0 is never returned as a valid offset.
  ...
  std::string_view Data(uint32_t pos) {
    return {data_ + pos,
            *reinterpret_cast<uint32_t*>(data_ + pos - sizeof(uint32_t))};
  }
