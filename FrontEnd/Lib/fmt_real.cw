...
pub fun FmtE@(val r64, precision uint, force_sign bool, out span!(u8)) uint:
    ...
    if num_real::r64_raw_exponent(val) == num_real::r64_raw_exponent_subnormal:
        if num_real::r64_raw_mantissa(val) == 0:
            ...
            set i += FmtMantissaE(
                    span(front(buffer), 1), precision, span_inc_or_die#(out, i))
            set i += FmtExponentE(0, span_inc_or_die#(out, i))
            return i
        return 0
    ...
    set i += FmtSign(is_negative, force_sign, out)
    set i += FmtMantissaE(
            span(front(buffer), num_digits), precision, span_inc_or_die#(out, i))
    set i += FmtExponentE(t, span_inc_or_die#(out, i))
...
pub fun FmtHex@(val r64, out span!(u8)) uint:
    ...
    set i += FmtExponentHex(raw_exponent, frac_bits == 0, span_inc_or_die#(
            out, i))
    return i
