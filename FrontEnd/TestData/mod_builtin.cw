...
(fun slice_copy [(param src (slice u8)) (param dst (slice mut u8))] uint [
    ...
    (return (call mymemcpy [(front mut dst) (front src) n]))
])


(fun nan_to_str [(param is_non_neg bool) (param frac_is_zero bool) (param out (slice mut u8))] uint [
      (if frac_is_zero [
         (if is_non_neg 
...
(fun r64_to_hex_str [(param val r64) (param out (slice mut u8))] uint [
   ...
   (if (== exp_bits 0x7ff) [(return (call nan_to_str [(== sign_bit 0) (== frac_bits 0) out]))] [])
...
(fun polymorphic SysRender [(param v r64_hex) 
                            (param out (slice mut u8)) 
                            (param options (ptr mut SysFormatOptions))] uint [
