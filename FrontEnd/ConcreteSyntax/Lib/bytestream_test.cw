...
fun test_bs_or_die() void:
    ...
    test::AssertEq#(0x22_u8, bytestream::FrontLeU8OrDie(&!stream))
...
fun test_bs() void:
    let! data [23]u8 = "\x22\x33\x44\x55\x66\x77\x88abcdefghijklmnop"
    @ref let! stream slice(u8) = data
    test::AssertEq#(0x22_u8, bytestream::FrontLeU8(&!stream))
    ...
    let dummy1 = typeid(slice(u8))
    let dummy2 = typeid(bytestream::OutOfBoundsError)
