...
@polymorphic fun SysRender(v bool, buffer slice!(u8), options ^!SysFormatOptions) 
uint:
    let s = v ?? as("true", slice(u8)) !! as("false", slice(u8))
    let n uint = len(buffer) min len(s)
    return mymemcpy(front!(buffer), front(s), n)

macro unsigned_to_str# EXPR($val EXPR, $base EXPR, $max_width EXPR, $out EXPR)[
    $v, $out_eval, $tmp, $pos]:
    expr:
...
fun u8_to_str(v u8, out slice!(u8)) uint:
    return unsigned_to_str#(v, 10, 32_uint, out)

fun u16_to_str(v u16, out slice!(u8)) uint:
    return unsigned_to_str#(v, 10, 32_uint, out)

fun u32_to_str(v u32, out slice!(u8)) uint:
    return unsigned_to_str#(v, 10, 32_uint, out)

fun u64_to_str(v u64, out slice!(u8)) uint:
    return unsigned_to_str#(v, 10, 32_uint, out)

fun s32_to_str(v s32, out slice!(u8)) uint:
    if len(out) == 0:
        return 0
    if v < 0:
...
@polymorphic fun SysRender(v u8, out slice!(u8), options ^!SysFormatOptions) uint:
    return u8_to_str(v, out)

@polymorphic fun SysRender(v u16, out slice!(u8), options ^!SysFormatOptions) uint:
    return u16_to_str(v, out)

@polymorphic fun SysRender(v u32, out slice!(u8), options ^!SysFormatOptions) uint:
    return u32_to_str(v, out)

@polymorphic fun SysRender(v u64, out slice!(u8), options ^!SysFormatOptions) uint:
    return u64_to_str(v, out)

@polymorphic fun SysRender(v s32, out slice!(u8), options ^!SysFormatOptions) uint:
    return s32_to_str(v, out)

@polymorphic fun SysRender(
    v slice(u8), buffer slice!(u8), options ^!SysFormatOptions) uint:
...
fun uint_to_hex_str(v uint, out slice!(u8)) uint:
    return unsigned_to_str#(v, 16, sizeof(uint) * 8, out)

fun u64_to_hex_str(v u64, out slice!(u8)) uint:
    return unsigned_to_str#(v, 16, 64_uint, out)

fun u32_to_hex_str(v u32, out slice!(u8)) uint:
    return unsigned_to_str#(v, 16, 32_uint, out)

fun u16_to_hex_str(v u16, out slice!(u8)) uint:
    return unsigned_to_str#(v, 16, 32_uint, out)

fun u8_to_hex_str(v u8, out slice!(u8)) uint:
    return unsigned_to_str#(v, 16, 32_uint, out)

@polymorphic fun SysRender(v uint_hex, out slice!(u8), options ^!SysFormatOptions) 
uint:
    return uint_to_hex_str(unwrap(v), out)

@polymorphic fun SysRender(v u64_hex, out slice!(u8), options ^!SysFormatOptions) 
uint:
    return u64_to_hex_str(unwrap(v), out)

@polymorphic fun SysRender(v u32_hex, out slice!(u8), options ^!SysFormatOptions) 
uint:
    return u32_to_hex_str(unwrap(v), out)

@polymorphic fun SysRender(v u16_hex, out slice!(u8), options ^!SysFormatOptions) 
uint:
    return u16_to_hex_str(unwrap(v), out)

@polymorphic fun SysRender(v u8_hex, out slice!(u8), options ^!SysFormatOptions) 
uint:
    return u8_to_hex_str(unwrap(v), out)

@pub @wrapped type rune = u8

@polymorphic fun SysRender(v rune, buffer slice!(u8), options ^!SysFormatOptions) 
uint:
    if len(buffer) == 0:
        return 0
    else:
...
fun r64_to_hex_str(val r64, out slice!(u8)) uint:
    let val_bits = asbits(val, s64)
    ...
    set i += u64_to_str(as(exp, u64), rest)
    return i

@pub @wrapped type r64_hex = r64

@polymorphic fun SysRender(v r64_hex, out slice!(u8), options ^!SysFormatOptions) 
uint:
    return r64_to_hex_str(unwrap(v), out)

@pub @wrapped type str_hex = slice(u8)

@polymorphic fun SysRender(v str_hex, out slice!(u8), options ^!SysFormatOptions) 
uint:
...
@polymorphic fun SysRender(v ^void, out slice!(u8), options ^!SysFormatOptions) uint:
    let h = wrapas(asbits(v, uint), uint_hex)
    return @polymorphic SysRender(h, out, options)

@pub macro print# STMT_LIST($parts EXPR_LIST_REST)[$buffer, $curr, $options]:
    mlet! $buffer =[FORMATED_STRING_MAX_LEN] u8{}
    mlet! $curr uint = 0
    @ref mlet! $options = SysFormatOptions{}
    mfor $i $parts:
        set $curr += @polymorphic SysRender(
            $i, slice(pinc(front!($buffer), $curr), len($buffer) - $curr), &!$options)
