...
; input modes
pub global IGNBRK  u32 =0x001
pub global BRKINT u32 = 0x002
pub global IGNPAR u32 = 0x004
pub global PARMRK u32 = 0x008
pub global INPCK u32 =  0x010
pub global ISTRIP u32 = 0x020
pub global INLCR  u32 = 0x040
pub global IGNCR  u32 = 0x080
pub global ICRNL u32 = 0x100
...
; output modes
pub global OPOST u32 = 0x01
pub global OLCUC u32 = 0x02
pub global ONLCR u32 = 0x04
pub global ONOCR u32 =   0x10
pub global ONLRET u32 =  0x20
pub global OFILL u32 =   0x40
pub global OFDEL u32 =   0x80
...
pub fun TcGetattr(fd os\FD, arg ^Termios) union(void, os\Error):
    let res = os\ioctl(unwrap(fd), unwrap(IoctlOp.TCGETS), bitwise_as(arg, uint))
    if res < 0:
        return wrap_as(as(res, s32), os\Error)
    else:
        return void_val

pub fun TcSetattrNow(fd os\FD, arg ^Termios) union(void, os\Error):
    let res = os\ioctl(unwrap(fd), unwrap(IoctlOp.TCSETS), bitwise_as(arg, uint))
    if res < 0:
        return wrap_as(as(res, s32), os\Error)
    else:
        return void_val

pub fun TcSetattrDrain(fd os\FD, arg ^Termios) union(void, os\Error):
    let res = os\ioctl(unwrap(fd), unwrap(IoctlOp.TCSETSW), bitwise_as(arg, uint))
    if res < 0:
        return wrap_as(as(res, s32), os\Error)
    else:
        return void_val

pub fun TcSetattrFlush(fd os\FD, arg ^Termios) union(void, os\Error):
    let res = os\ioctl(unwrap(fd), unwrap(IoctlOp.TCSETSF), bitwise_as(arg, uint))
    if res < 0:
        return wrap_as(as(res, s32), os\Error)
    else:
        return void_val
