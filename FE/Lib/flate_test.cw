...
global AllTestCases = {
        [27]TestCase: {
            TestCase: "generic: missing next block after final uncompressed block",
            {[5]u8: 0x00, 0x00, 0x00, 0xff, 0xff},
            flate::TruncationErrorVal,
            "",
            large_output_buffer},
        {
            TestCase: "generic: invalid block 11",
            {[1]u8: 0x07},
            flate::CorruptionErrorVal,
            "",
            large_output_buffer},
        {
            TestCase: "uncompressed: truncation",
            {[1]u8: 0x01},
            flate::TruncationErrorVal,
            "",
            large_output_buffer},
            ...
            large_output_buffer},
        {
            ...
            {
                [37]u8: 0x0b, 0x4f, 0x55, 0x28, 0xc9, 0x48, 0x55, 0x08, 0x48, 0xcd, 
                0x2f, 0xc8, 0x49, 0x55, 0xc8, 0x4f, 0x03, 0xf3, 0x42, 0xf3, 0x32, 
                0x4b, 0x52, 0x53, 0x14, 0x82, 0x4b, 0x12, 0x4b, 0x52, 0x8b, 0x75, 
        ...
        ; dynamic huffman: =======================================
        {
            TestCase: "dynamic huffman:  256 zero bytes compressed using RLE (only one distance code)",
            {
            ...
            large_output_buffer},
        {
            ...
            {
                [13]u8: 0x05, 0xca, 0x81, 0x00, 0x00, 0x00, 0x00, 0x00, 0x90, 0xff, 
                0x6b, 0x01, 0x00},
            0_uint,
            "",
            large_output_buffer},
        {
            TestCase: "dynamic huffman:  256 zero bytes (no distance only literal tree)",
            {
                [45]u8: 0x05, 0xca, 0x81, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0xff, 
                0xd5, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
            ...
            large_output_buffer},
        {
            ...
            {
                [53]u8: 0xed, 0xdd, 0x01, 0x01, 0x00, 0x00, 0x08, 0x02, 0x20, 0xed, 
                0xff, 0xe8, 0xfa, 0x11, 0x1c, 0x61, 0x9a, 0xf7, 0x00, 0x00, 0x00, 
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xe0, 0xfe, 0xff, 0x05},
            32771_uint,
            special_2_1_0,
            large_output_buffer},
        {
            TestCase: "dynamic huffman:  4 zero bytes - use code length codes include codes 16, 17, and 18",
            {
            ...
            large_output_buffer},
        {
            ...
            {
                [39]u8: 0x05, 0xea, 0x01, 0x82, 0x24, 0x49, 0x92, 0x24, 0x49, 0x02, 
                0x12, 0x8b, 0x9a, 0x47, 0x56, 0xcf, 0xde, 0xff, 0x9f, 0x7b, 0x0f, 
                0xd0, 0xee, 0x7d, 0xbf, 0xbf, 0x7f, 0xff, 0xfd, 0xef, 0xff, 0xfe, 
...
fun test_all() void:
    for i = 0, len(AllTestCases), 1:
        ...
        if is(res, uint):
            test::AssertSliceEq#(
                    tc^.expected_output, make_span(
                        front(tc^.output), narrow_as!(res, uint)))
