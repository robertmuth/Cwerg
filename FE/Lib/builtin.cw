...
pub {{builtin}} macro for STMT_LIST(
        $index ID, $start EXPR, $end EXPR, $step EXPR, $body STMT_LIST)[
...
pub {{builtin}} macro trylet STMT_LIST(
...
pub {{builtin}} macro span_inc EXPR($slice EXPR, $size EXPR)[
    ...
    expr:
        ...
        if $orig_size > $orig_len:
            trap
        return make_span(
                ptr_inc({{preserve_mut}} front($orig_slice), $orig_size), $orig_len -
                $orig_size)
