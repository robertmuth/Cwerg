...
NodeDesc GlobalNodeDescs[] = {
    ...
    {BIT_N(params) | BIT_N(result) | BIT_N(body), BIT_S(name),
     BIT_B(init) | BIT_B(fini) | BIT_B(pub) | BIT_B(ref) | BIT_B(poly) |
         BIT_B(externx) | BIT_B(cdecl)},  // DefFun
    {BIT_N(type_or_auto) | BIT_N(initial_or_undef_or_auto), BIT_S(name),
     BIT_B(pub) | BIT_B(mut) | BIT_B(ref) | BIT_B(cdecl)},  // DefGlobal
    {BIT_N(params_macro) | BIT_N(gen_ids) | BIT_N(body_macro), BIT_S(name),
     BIT_B(pub) | BIT_B(builtin)},                             // DefMacro
    ...
    {BIT_N(expr1) | BIT_N(expr2) | BIT_N(expr_bound_or_undef), 0,
     0},                                // ExprPointer
